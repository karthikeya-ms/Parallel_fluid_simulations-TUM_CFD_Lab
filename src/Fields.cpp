...
void Fields::calculate_fluxes(Grid &grid, double gamma) {
	...
	for (int i_idx = 1; i_idx < grid.imax()+1; i_idx++){
		for (int j_idx = 1; j_idx < grid.jmax()+1; j_idx++){
		         cu = discretization.convection_u(_U,_V,i_idx,j_idx,_nu);
		         cv = discretization.convection_v(_U,_V,i_idx,j_idx,_nu);
			_F(i_idx, j_idx) = _U(i_idx, j_idx) + _dt*(cu + _gx);	
			_G(i_idx, j_idx) = _V(i_idx, j_idx) + _dt*(cv + _gy);
		}
	}
...
double Fields::calculate_dt(Grid &grid) { 
	...
	for (auto i_idx = 0; i_idx < _imax+1; i_idx++){
		//std::cout << "Accessing row: " << i_idx << std::endl; 
		std::vector<double> row_u = _U.get_row(i_idx);
		std::vector<double> row_v = _V.get_row(i_idx);
		//std::cout << "Row is received\n"; 
		
		for(auto j_idx = 0; j_idx < _jmax+1; j_idx++){
		//std::cout << i_idx << " " << j_idx << " " << row_u.size() << " " << row_v.size() << " " << _U.size() << " " <<  _V.size() << std::endl;
