...
void treatBoundary(double *collideField, int* flagField,
                   const double * const wallVelocity, int xlength_lbm){
	...
	for (x = 0; x < len; x++){
			...
			for (z = 0; z < len; z++) {
				...
				currentCell = collideField + Q_NUMBER*index;
				...
				for (i = 0; i < Q_NUMBER; ++i) {
					...
					if ((x+dx > 0 && x+dx<len-1) && (y+dy > 0 && y+dy< len-1) && (z+dz > 0 && z+dz< len-1)){
						...
						if (flagField[index] == MOVING_WALL){
							...
							*(currentCell + i) = *(currentCell + Q_NUMBER*(dz*len*len + dy*len + dx) + Q_NUMBER-1-i) +
